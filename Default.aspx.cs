       ...
       if (file_photo.HasFile)
       {
           ...
           if (file[1] == "jpg" || file[1] == "jpeg" || file[1] == "png")
           {
               if (mb <= 1)
               {
                   ...
                   lbl_photo.Text = "image size must be less than 1 MB.";
           }
           else
               lbl_photo.Text = "File extension must be of (jpg,png,jpeg) type.";
           photopath = "~\\uploads\\" + filename;
       }
       string insert = "insert into register values (" + id + ",'" + uname + "','" + email + "','" + pass + "','" + add + "','" + photopath + "','" + mobile + "','"+ques+"','"+ans+"')";
